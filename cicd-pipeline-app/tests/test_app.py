...
def test_index_post_potencia(client):
    ...
    assert response.status_code == 200
    assert b'Resultado: 8.0' in response.data
...
def test_index_post_modulo(client):
    ...
    assert response.status_code == 200
    assert b'Resultado: 1.0' in response.data


def test_index_post_modulo_by_zero(client):
    response = client.post(
        '/', data={'num1': '10', 'num2': '0', 'operacion': 'modulo'})
    assert response.status_code == 200
    assert b'Error: No se puede calcular el m\xc3\xb3dulo por cero' in response.data
